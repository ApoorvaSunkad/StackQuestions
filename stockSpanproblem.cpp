...
given day is less than or equal to its price on the current day.
...
100 is greater than equal to 100 and there are no more elements behind it so the span 
is 1,
...
an array price[] denoting the price of stocks, and an integer N denoting the size 
of the array and number of days. This function finds the span of stock's price for 
all N days and returns an array of length N denoting the span for the i-th day.
...
//Optimised solution: Uses the concept of next greatest to left question where we find
// the next greatest ele to left of current element

//Here inside stack we are using a pair to store the next greatest ele and its index
